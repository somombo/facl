...
export const SIMPLE_RULE_EXAMPLE = `
service cloud.functions.https {
  match /apis/{api}/endpoint {
    match /{endpoint=**} {
...
/* 
  const MATCH_ANY = `(/*)?`
  export const example_acl : ACL = [
    new Rule('/app', [new Allow(['read'], _ => true)] ),
    new Rule( '/app/my_apis/:api/endpoints' ),
    new Rule(
      '/app/my_apis/:api/endpoints', 
      [ new Allow(['read', 'write'], (request: any) => request.auth.uid != null) ],
      true
    ),
    new Rule(
      '/app/other_api/:other_endpoint', 
      [
        new Allow(['get'], (request: Request) => !!request.accepts('json')),
        new Allow(
          ['post', 'put', 'delete'],
          (request: any) => request.body !== null
        )
      ] 
    )
  ]
...
//       "allow get":  request => request.accepts("json") as boolean,

//       "allow post,put,delete": request => request.body != null as boolean
//     }
//   }
...
/* 
  function parseUrls() {
    const urls = [
      '/app',
      '/app/my_apis/:api/endpoints',
      `/app/my_apis/:api/endpoints/${MATCH_ANY}`,
      '/app/other_api/:other_endpoint'
    ]

    const ptr = require('path-to-regexp')
    urls.forEach(u => {
      console.log(ptr.parse(u))
    })
  } 
*/